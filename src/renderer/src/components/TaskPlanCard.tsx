  ...
  const steps = rawSteps
    ...
    .map((step: any, index: number) => ({
      ...
      dependencies: Array.isArray(step?.dependencies)
        ? step.dependencies.map(String).filter(Boolean)
        : [],
      ...
      artifactPaths: Array.isArray(step?.artifactPaths)
        ? step.artifactPaths.map(String).filter(Boolean)
        : [],
      retryCount: Math.max(0, Number(step?.retryCount) || 0),
      agentRole: ['general', 'researcher', 'coder', 'reviewer'].includes(
        String(step?.agentRole || step?.role)
      )
...
export function latestTaskPlan(toolSteps: any[]): TaskPlan | null {
  for (let index = toolSteps.length - 1; index >= 0; index -= 1) {
    ...
    if (step?.type === 'call' && ['update_task_plan', 'delegate_tasks'].includes(step?.name)) {
      const source =
        step.name === 'delegate_tasks'
          ? {
              title: step.detail?.title || step.detail?.goal || 'Delegated tasks',
              explanation: 'Live multi-agent execution graph',
              steps: step.detail?.tasks || step.detail?.subtasks || []
            }
          : step.detail
      const plan = normalizeTaskPlan(source)
      if (!plan) return null
      const result = toolSteps
        .slice(index + 1)
        .find((candidate: any) => candidate?.type === 'result' && candidate?.name === step.name)
      if (typeof result?.detail === 'string') {
        try {
          plan.runId = String(JSON.parse(result.detail)?.taskRunId || '') || undefined
        } catch {
          /* Historical result did not contain a run id. */
        }
      }
      return plan
    }
...
export const TaskPlanFloatingStatus = React.memo(function TaskPlanFloatingStatus({
  plan,
  messageId
}: {
  plan: TaskPlan
  ...
  useEffect(() => {
    let active = true
    const accepts = (run: any): boolean =>
      Boolean(
        run &&
        ((plan.runId && run.id === plan.runId) ||
          (messageId !== undefined && String(run.messageId) === String(messageId)))
      )
    const applySnapshot = (run: any, steps: any[]): void => {
      ...
      setRuntimePlan({
        ...
        explanation: run.explanation || plan.explanation,
    ...
    const unsubscribe = window.api.onTaskRunUpdated((update: any) =>
      applySnapshot(update?.run, update?.steps)
    )
    return () => {
      active = false
      unsubscribe()
  ...
  const currentStepIndex = displayPlan.steps.findIndex((step) => step.status === 'in_progress')
  if (currentStepIndex < 0) return null

  const currentStep = displayPlan.steps[currentStepIndex]
  const completed = displayPlan.steps.filter((step) => step.status === 'completed').length
  const progress = Math.max(8, Math.round((completed / displayPlan.steps.length) * 100))
  ...
  return (
    ...
    >
      ...
      <span className="task-plan-floating-count">
        第 {currentStepIndex + 1} / {displayPlan.steps.length} 步
      </span>
      <span className="task-plan-floating-divider" aria-hidden="true">
        ·
      </span>
...
function StepMarker({ status }: { status: TaskStepStatus }): React.JSX.Element {
  ...
  if (status === 'in_progress')
    return <LoaderCircle size={14} strokeWidth={2.2} aria-hidden="true" />
...
function layoutDag(steps: TaskPlanStep[]): { nodes: DagNode[]; width: number; height: number } {
  ...
  const levelOf = (id: string, visiting = new Set<string>()): number => {
    ...
    const dependencies = (byId.get(id)?.dependencies || []).filter((dependency) =>
      byId.has(dependency)
    )
    const level =
  ...
  return {
    nodes,
    width: padding * 2 + columnCount * nodeWidth + (columnCount - 1) * columnGap,
    height
  }
}
...
function ReadonlyTaskNodeView({ data }: NodeProps<ReadonlyTaskNode>): React.JSX.Element {
  ...
  return (
    <article
      className={`task-dag-node is-${step.status} ${highlighted ? 'is-selected' : ''}`}
      style={{ position: 'relative', width: 135, height: 54 }}
    ...
    >
      ...
      <span className="task-dag-node-top">
        <span className="task-dag-node-marker">
      ...
      <strong>{step.title}</strong>
      <span className="task-dag-node-state" title={step.detail}>
        {step.status === 'in_progress' && step.detail ? step.detail : STATUS_LABEL[step.status]}
      </span>
      <Handle
        type="source"
...
export function TaskDagGraph({
  ...
  const nodes = useMemo<ReadonlyTaskNode[]>(
    () =>
  ...
  return (
    <section
      className={`task-dag task-dag-react-flow ${!showHeader ? 'is-bare' : ''}`}
      aria-label={`只读流程图：${plan.title}`}
    >
      {showHeader && (
        <header className="task-dag-header">
          <span>
            <Network size={14} aria-hidden="true" />
            <strong>Agent DAG</strong>
          </span>
          <small>
            {running > 0 ? `${running} running` : `${completed}/${plan.steps.length} complete`}
          </small>
        </header>
      )}
      <div
        className="task-dag-flow-viewport"
        tabIndex={0}
        aria-label="流程图画布，滚轮缩放，拖动空白处移动；Ctrl 或 Command 加减键缩放，0 适应画布"
        onPointerDown={(event) => {
          if (!(event.target as HTMLElement).closest('button, .react-flow__node'))
...
export const TaskPlanCard = React.memo(function TaskPlanCard({
  toolSteps,
  messageId
}: {
  toolSteps: any[]
  ...
  return (
    <section
      className={`task-plan-card ${blocked ? 'is-blocked' : ''} ${allCompleted ? 'is-complete' : ''}`}
    ...
    >
      ...
      >
        <span className="task-plan-symbol">
          <ListChecks size={16} strokeWidth={2.1} aria-hidden="true" />
        </span>
      ...
      {expanded && (
        <div className="task-plan-body">
          {displayPlan.explanation && (
            <p className="task-plan-explanation">{displayPlan.explanation}</p>
          )}
...
export const TaskPlanPanel = React.memo(function TaskPlanPanel({
  plan,
  messageId,
  running,
  onClose,
  ...
  const showDag = steps.some(
    (step) => step.agentId !== 'agentpet' || step.agentRole || (step.dependencies?.length || 0) > 0
  )
  ...
  useEffect(() => {
    let active = true
    const matches = (run: any): boolean =>
      Boolean(
        run &&
        ((runtimeRunId && run.id === runtimeRunId) ||
          (plan.runId && run.id === plan.runId) ||
          (messageId !== undefined && String(run.messageId) === String(messageId)))
      )
    const applySnapshot = async (snapshot: any): Promise<void> => {
      ...
      const runs = snapshot.run.sessionId
        ? await window.api.listTaskRuns(snapshot.run.sessionId)
        : []
      if (!active) return
  ...
  const control = async (action: 'pause' | 'resume' | 'cancel'): Promise<void> => {
    ...
    try {
      await window.api.controlTaskRun(runtimeRunId, action)
    } finally {
  ...
  const retry = async (stepId: string): Promise<void> => {
    ...
    setIsControlling(true)
    try {
      await window.api.retryTaskStep(runtimeRunId, stepId)
    } finally {
      setIsControlling(false)
    }
  }
  ...
  return (
    <aside
      className={`task-plan-side-panel ${blocked ? 'is-blocked' : ''} ${allCompleted ? 'is-complete' : ''}`}
      aria-label={`任务执行面板：${plan.title}`}
    >
      <header className="task-plan-panel-header">
        <span className="task-plan-panel-icon">
      ...
      {runtimeRunId && !allCompleted && (
        <div className="task-plan-panel-controls" aria-label="Task controls">
          <Tooltip content="暂停执行并保存检查点" placement="top">
            <button type="button" disabled={isControlling} onClick={() => control('pause')}>
              <Pause size={14} />
              暂停
            </button>
          </Tooltip>
          <Tooltip content="继续执行任务" placement="top">
            <button type="button" disabled={isControlling} onClick={() => control('resume')}>
              <Play size={14} />
              继续
            </button>
          </Tooltip>
          <Tooltip content="终止任务执行" placement="top">
            <button type="button" disabled={isControlling} onClick={() => control('cancel')}>
              <Square size={13} />
              停止
            </button>
          </Tooltip>
      ...
      <div className="task-plan-panel-scroll">
        ...
        <ol className="task-plan-panel-steps">
          {steps.map((step, index) => (
            <li key={step.id} className={`is-${step.status}`}>
              <span className="task-plan-panel-index">
                <StepMarker status={step.status} />
              </span>
              <span className="task-plan-panel-step-copy">
                ...
                {runtimeRunId && step.status === 'blocked' && (
                  <button
                    className="task-plan-step-retry"
                    type="button"
        ...
        {relatedRuns.length > 1 && (
          <section className="task-plan-related-runs">
            ...
            {relatedRuns.map((item) => {
              const done = Array.isArray(item.steps)
                ? item.steps.filter((step: any) => step.status === 'completed').length
                : 0
              ...
              return (
                <div key={item.run.id} className={`is-${item.run.status}`}>
                  <span>
                    <b>{item.run.title}</b>
                    <small>{item.run.status}</small>
                  </span>
                  <small>
                    {done} / {total}
                    {item.run.id === runtimeRunId ? ' · current' : ''}
                  </small>
                </div>
              )
            })}
