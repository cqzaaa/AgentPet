...
`
    ...
    if (name === 'web_search') {
      try {
        const { query } = args
        if (!query) return '错误：缺少必要参数 query'
        ...
        if (!response.ok) {
          ...
          return `联网搜索失败：Jina.ai 搜索服务返回状态码 ${response.status}${errMsg ? ' - ' + errMsg : ''}`
        }
        const data = await response.text()
        return data || `未找到与 "${query}" 相关的搜索结果。`
      } catch (err: any) {
        return `联网搜索执行失败：${err.message || err}`
      }
    }
    ...
    if (name === 'web_fetch') {
      try {
        ...
        if (!textContent || !textContent.trim()) {
          return '成功加载网页，但本地未提取到任何正文文本。'
        }

        // 截取前 12000 个字符以防超过模型上下文限制
        return textContent.trim().slice(0, 12000)
      } catch (err: any) {
        return `网页抓取失败：${err.message || err}`
      }
    }
